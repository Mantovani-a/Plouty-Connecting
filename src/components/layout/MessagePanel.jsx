...
export default function MessagePanel({ conversations, isOpen, onOpen, onClose, targetBusinessKey }) {
    ...
    return (
      <div id="plouty-messages-panel" className="message-dock-collapsed">
        ...
        >
          ...
          <span><strong>Mensagens</strong><small>Conversas comerciais</small></span>
  ...
  const panel = (
    <section
      ...
      ref={panelRef}
      className={`message-dock is-open ${selectedConversation ? 'showing-thread' : 'showing-list'}`}
    ...
    >
      ...
      {!selectedConversation ? (
        <div className="message-conversation-list" aria-label="Lista de conversas">
          <div className="message-demo-note"><i className="bi bi-info-circle" aria-hidden="true" /><span><strong>Experiência demonstrativa</strong> Estas conversas são dados fictícios e estáveis do protótipo.</span></div>
          {!conversations.length && (
            <div className="message-empty-state">
              ...
              <p>Quando uma negociação iniciar, o contato relacionado aparecerá nesta lista.</p>
            </div>
          )}
          {conversations.map((conversation) => (
            <button
              key={conversation.id}
              type="button"
              className={`conversation-item ${conversation.unread ? 'is-unread' : ''}`}
            ...
            >
              ...
              <span className="conversation-preview">
                <span><strong>{conversation.name}</strong>{conversation.verified && <i className="bi bi-patch-check-fill" aria-label="Perfil verificado" />}</span>
                <small>{conversation.lastMessage}</small>
              </span>
              <span className="conversation-meta"><time>{conversation.time}</time>{conversation.unread > 0 && <i aria-label="Mensagem não lida" />}</span>
            </button>
          ))}
        </div>
      ) : (
        <div className="message-thread-view">
          <div ref={threadRef} className="message-thread" aria-label={`Conversa com ${selectedConversation.name}`}>
            <div className="thread-context"><i className="bi bi-shield-check" aria-hidden="true" /><span>Conversa demonstrativa sobre oportunidades, contratos e entregas.</span></div>
            {threadMessages.map((message) => (
              <div key={message.id} className={`thread-message is-${message.sender}`}>
                <p>{message.body}</p>
